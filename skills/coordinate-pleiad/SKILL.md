...
6. Before every issue-backed task creation call `scripts.coordination_protocol.dispatch_issue_task`; before every required cross-task send call `send_cross_task_handoff`. Resolve the effective project policy to explicit final model/effort values, query the destination host's currently available model/effort routes, and pass those routes to the helper. It rejects unavailable requested routes before transport and never substitutes a model. Pass a one-sentence `rationale` explicitly when creating or activating every durable handoff. Built-in defaults keep Luna for straightforward implementation, Terra for nontrivial implementation, Sol for difficult implementation and coordination/design/QA/review. Astra is never a routine default and needs a permitted enabled policy route plus a structured exceptional escalation explaining why Sol is insufficient.
