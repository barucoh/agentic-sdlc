...
At first adoption, offer one concise choice between recommended defaults and customization unless the user already chose. For a customization, pass `--routing-policy <complete-json-file>` to the same dry-run/apply commands; it validates before writing `.pleiad/model-routing.json`, which is project-owned and preserved by upgrades. Every durable handoff explicitly passes `target_model`, `effort`, and a one-sentence `rationale`; canonical validation consumes the effective policy. The recommended Astra route is exceptional/High effort; projects may explicitly permit another supported effort, but Astra is never a routine default and can be disabled.
