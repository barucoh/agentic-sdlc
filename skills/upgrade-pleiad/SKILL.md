...
The upgrade preserves the project-owned `.pleiad/model-routing.json` when present; absent overrides use safe built-in defaults. Every durable handoff must explicitly pass `target_model`, `effort`, and a one-sentence `rationale`; invalid policy or role/model/effort pairs are rejected before dispatch, and write-capable ephemeral research remains a conflict.
