...
QA verification uses an isolated disposable workspace-write worktree derived from the exact implementation commit. Behavioral tools may create caches/build/test outputs, but QA must not mutate source, commit, push, or alter the Implementation worktree/branch. The repository QA workspace check records source status and diff before and after, while Codex host/task-control provisions and removes the disposable worktree; the repository does not claim to provision or delete it. QA reports blocked on drift or unavailable cleanup.
